...
class DepartmentTransferService {
  ...
  static List<DepartmentTransferRequirement> requirementsFromGoal(
    CareerGoal goal,
  ) {
    return goal.requirements
        .where((r) => r.defaultRequired)
        .map(
          (r) => DepartmentTransferRequirement(
    ...
    final items = plan.requirements.map((requirement) {
      ...
      if (requirement.kind == TransferRequirementKind.certification) {
        ...
        if (matchingCerts.isNotEmpty) {
          return TransferRequirementEvaluation(
            ...
            satisfied: true,
            reason:
                'Matched current credential: ${app.certificationDisplayName(matchingCerts.first)}.',
            matchingRecords: 0,
          );
        }
      }
      ...
      if (matchedRecords.isNotEmpty) {
        return TransferRequirementEvaluation(
          ...
          satisfied: true,
          reason:
              'Found ${matchedRecords.length} matching career record${matchedRecords.length == 1 ? '' : 's'} that may support this requirement.',
      ...
      if (progressMatch) {
        return TransferRequirementEvaluation(
          ...
          satisfied: true,
          reason:
              'Found retained Task Book / path progress that may support this requirement.',
          matchingRecords: 0,
        );
      }
      ...
      return TransferRequirementEvaluation(
        ...
        satisfied: false,
        reason:
            'No current credential or clearly matching career evidence was found. Verify this item with the target department.',
        matchingRecords: 0,
      );
    }).toList();
  ...
  static String buildComparisonText(DepartmentTransferEvaluation evaluation) {
    ...
    buffer.writeln(
      plan.departmentName.trim().isEmpty
          ? 'Target department not named'
    ...
    buffer.writeln(
      'Estimated overlap: ${evaluation.satisfiedCount}/${evaluation.totalCount} requirements (${(evaluation.percent * 100).round()}%)',
    );
    buffer.writeln();
    ...
    if (evaluation.gaps.isEmpty) {
      buffer.writeln(
        '- No obvious gaps identified from the information entered.',
      );
    } else {
    ...
    buffer.writeln();
    buffer.writeln(
      'This comparison is a personal planning aid. The receiving department or certifying authority determines what transfers, what must be repeated, and whether supporting documentation is acceptable.',
    );
    return buffer.toString().trim();
  }

  static TransferRequirementKind _kindForRequirement(RequirementType type) =>
      switch (type) {
        RequirementType.certification => TransferRequirementKind.certification,
        RequirementType.experience ||
        RequirementType.numericProgress => TransferRequirementKind.experience,
        RequirementType.taskBook => TransferRequirementKind.taskBook,
        RequirementType.education ||
        RequirementType.trainingCourse ||
        RequirementType.course => TransferRequirementKind.education,
        RequirementType.practical ||
        RequirementType.promotionalTest ||
        RequirementType.interview => TransferRequirementKind.practical,
        RequirementType.custom => TransferRequirementKind.other,
      };

  static Iterable<String> _candidateTerms(
    ...
    if (defId != null) {
      ...
      if (def != null) {
        values.addAll([
          def.displayName,
          if (def.shortName != null) def.shortName!,
          ...def.aliases,
        ]);
      }
    }
