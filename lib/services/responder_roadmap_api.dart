...
class ResponderRoadmapApi {
  ...
  Future<void> registerCurrentMemberForClassQr(String qrValue) async {
    final uri = Uri.tryParse(qrValue.trim());
    if (uri == null ||
        uri.host.toLowerCase() != 'responderroadmap.com' ||
        uri.pathSegments.length != 2 ||
        uri.pathSegments.first != 'class-register') {
      throw const ResponderRoadmapApiException('This is not a Responder Roadmap class QR code.');
    }
  ...
  Map<String, dynamic> _asMap(Object? value) {
    ...
    return <String, dynamic>{};
      ...
      throw const ResponderRoadmapApiException('This class QR code is invalid or expired.');
    }
    final session = await currentSession();
    if (!session.hasDepartment || session.email.trim().isEmpty || session.name.trim().isEmpty) {
      throw const ResponderRoadmapApiException('Your department profile is incomplete. Sign in again before scanning.');
    }
    await _request(
      'POST',
      'public/classes/${Uri.encodeComponent(token)}',
      authenticated: false,
      body: <String, dynamic>{
        'name': session.name.trim(),
        'email': session.email.trim(),
        'organization': session.departmentName?.trim() ?? '',
        'consent': true,
        'website': '',
      },
    );
