...
class CareerExportIdentity {
  ...
  factory CareerExportIdentity.empty() => const CareerExportIdentity(
        name: '',
        email: '',
        phone: '',
        location: '',
...
class CareerPdfExport {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          _body('Active goal: ${app.selectedGoal?.title ?? 'No active goal selected'}'),
          ...
          _body('Readiness score: ${advancement.readinessScore}% — ${advancement.readinessLabel}'),
          _body('Requirements complete: ${advancement.completedRequirements}/${advancement.totalRequirements}'),
          _body('Evidence coverage: ${advancement.evidenceCovered}/${advancement.evidenceExpected}'),
          ...
          ..._recordBullets(records.where((r) =>
              r.type == CareerRecordType.leadership ||
              r.type == CareerRecordType.teaching ||
              r.type == CareerRecordType.project).take(18)),
          pw.SizedBox(height: 12),
          _sectionTitle('Development Priorities'),
          _body('${advancement.recommendation.title}: ${advancement.recommendation.reason}'),
          if (advancement.evidenceGaps.isNotEmpty)
            ...advancement.evidenceGaps.take(8).map((gap) => _bullet(gap.requirement.name)),
          pw.SizedBox(height: 12),
          _disclaimer(),
        ],
      ),
  ...
  static Future<Uint8List> buildResume({
    ...
    final highlights = records.where((r) => r.highlight || r.type == CareerRecordType.achievement || r.type == CareerRecordType.project).toList()
      ..sort((a, b) => b.date.compareTo(a.date));
    final leadership = records.where((r) => r.type == CareerRecordType.leadership || r.type == CareerRecordType.teaching).toList()
      ..sort((a, b) => b.date.compareTo(a.date));
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          pw.Text(
            identity.name.trim().isEmpty ? 'FIRE / EMS PROFESSIONAL' : identity.name.trim().toUpperCase(),
            style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: _navy),
          ),
          pw.SizedBox(height: 4),
          pw.Text(_contactLine(identity), style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700)),
          ...
          _body(_roles(app)),
          if ((app.profile.departmentName ?? '').trim().isNotEmpty) _body(app.profile.departmentName!.trim()),
          if (app.selectedGoal != null) _body('Career development target: ${app.selectedGoal!.title}'),
          ...
          pw.Wrap(
            ...
            children: app.certifications
                ...
                .take(18)
                .map((c) => pw.Text('• ${app.certificationDisplayName(c)}', style: const pw.TextStyle(fontSize: 9.5)))
                .toList(),
          ),
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          ...advancement.competencies.map((c) => _bullet('${c.title}: ${c.exampleCount}/${c.targetExamples} examples${c.supported ? ' — supported' : ' — build more evidence'}')),
          pw.SizedBox(height: 12),
          _sectionTitle('Evidence Gaps'),
          if (advancement.evidenceGaps.isEmpty)
            _bullet('No current roadmap evidence gaps identified.')
          else
            ...advancement.evidenceGaps.take(12).map((g) => _bullet(g.requirement.name)),
          pw.SizedBox(height: 12),
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.SizedBox(height: 12),
          _body('Target department: ${evaluation.plan.departmentName.trim().isEmpty ? 'Not specified' : evaluation.plan.departmentName.trim()}'),
          if ((evaluation.plan.targetRole ?? '').trim().isNotEmpty) _body('Target role: ${evaluation.plan.targetRole}'),
          ...
          ...evaluation.items.where((e) => e.satisfied).map((e) => _statusRow(e.requirement.title, e.reason, true)),
          pw.SizedBox(height: 12),
          ...
          if (evaluation.gaps.isEmpty)
            ...
            ...evaluation.gaps.map((e) => _statusRow(e.requirement.title, e.reason, false)),
          pw.SizedBox(height: 12),
          _body(DepartmentTransferService.buildComparisonText(evaluation).split('\n').last),
        ],
      ),
    );
    return doc.save();
  ...
  static pw.Widget _header(CareerExportIdentity identity, String title, AppState app) {
    return pw.Container(
        ...
        children: [
          pw.Expanded(
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text(title, style: pw.TextStyle(color: PdfColors.white, fontSize: 18, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 4),
                pw.Text(identity.name.trim().isEmpty ? _roles(app) : identity.name.trim(), style: pw.TextStyle(color: _gold, fontSize: 11, fontWeight: pw.FontWeight.bold)),
                if (_contactLine(identity).isNotEmpty)
                  pw.Text(_contactLine(identity), style: const pw.TextStyle(color: PdfColors.white, fontSize: 8.5)),
              ],
            ),
          ),
  ...
  static pw.Widget _resumeHeading(String text) => pw.Padding(
        ...
        child: pw.Container(
          ...
          decoration: pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(color: _gold, width: 1))),
          child: pw.Text(text, style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: _navy)),
        ),
      );
  ...
  static pw.Widget _bullet(String text) => pw.Padding(
        ...
        child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          ...
          pw.Expanded(child: pw.Text(text, style: const pw.TextStyle(fontSize: 9.3, lineSpacing: 2))),
        ]),
      );
  ...
  static Iterable<pw.Widget> _recordBullets(Iterable<CareerRecord> records) {
    ...
    return list.map((r) => _bullet('${_date(r.date)} — ${r.title}${(r.impact ?? '').trim().isEmpty ? '' : ': ${r.impact!.trim()}'}'));
  }

  static Iterable<pw.Widget> _resumeRecords(Iterable<CareerRecord> records) => records.map((r) => pw.Padding(
        padding: const pw.EdgeInsets.only(bottom: 5),
        child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Text(r.title, style: pw.TextStyle(fontSize: 9.5, fontWeight: pw.FontWeight.bold)),
          if ((r.roleOrAssignment ?? '').trim().isNotEmpty)
            pw.Text(r.roleOrAssignment!.trim(), style: const pw.TextStyle(fontSize: 8.5, color: PdfColors.grey700)),
          if ((r.impact ?? '').trim().isNotEmpty)
            pw.Text(r.impact!.trim(), style: const pw.TextStyle(fontSize: 8.8, lineSpacing: 2)),
        ]),
      ));
  ...
  static pw.Widget _storyBlock(CareerRecord record) => pw.Container(
        ...
        child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Text(record.title, style: pw.TextStyle(fontSize: 9.5, fontWeight: pw.FontWeight.bold, color: _navy)),
          if ((record.roleOrAssignment ?? '').trim().isNotEmpty) _body('Role: ${record.roleOrAssignment!.trim()}'),
          if ((record.summary ?? '').trim().isNotEmpty) _body('Situation / Action: ${record.summary!.trim()}'),
          if ((record.impact ?? '').trim().isNotEmpty) _body('Result: ${record.impact!.trim()}'),
        ]),
      );

  static pw.Widget _statusRow(String title, String reason, bool satisfied) => pw.Container(
        margin: const pw.EdgeInsets.only(bottom: 5),
        padding: const pw.EdgeInsets.all(7),
        decoration: pw.BoxDecoration(color: satisfied ? PdfColor.fromHex('#EEF7F0') : PdfColor.fromHex('#FFF4EC'), borderRadius: pw.BorderRadius.circular(4)),
        child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Text(satisfied ? '✓' : '○', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: satisfied ? PdfColors.green700 : PdfColors.orange700)),
          pw.SizedBox(width: 6),
          pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text(title, style: pw.TextStyle(fontSize: 9.3, fontWeight: pw.FontWeight.bold)),
            pw.Text(reason, style: const pw.TextStyle(fontSize: 8.3, color: PdfColors.grey700)),
          ])),
        ]),
      );
  ...
  static pw.Widget _disclaimer() => pw.Container(
        ...
        child: pw.Text('Personal professional-development document generated by FireOps Career Road. Verify official credentials, personnel records, promotional eligibility, and department requirements with the appropriate authority.', style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey700)),
      );

  static pw.Widget _footer(pw.Context context) => pw.Row(
        mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
        children: [
          pw.Text('FireOps Career Road', style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey600)),
          pw.Text('Page ${context.pageNumber} of ${context.pagesCount}', style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey600)),
        ],
      );

  static String _roles(AppState app) => app.profile.currentRoles.isEmpty ? 'Fire / EMS professional' : app.profile.currentRoles.join(' / ');

  static String _contactLine(CareerExportIdentity identity) => [identity.location, identity.phone, identity.email].where((e) => e.trim().isNotEmpty).join(' • ');

  static String _profileSummary(AppState app, List<CareerRecord> records) {
    final years = app.profile.yearsOfService;
    final leadership = records.where((r) => r.type == CareerRecordType.leadership || r.type == CareerRecordType.teaching || r.type == CareerRecordType.project).length;
  ...
  static String _developmentSummary(List<CareerRecord> records) {
    final training = records.where((r) => r.type == CareerRecordType.training).length;
    final education = records.where((r) => r.type == CareerRecordType.education).length;
